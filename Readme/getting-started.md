...
### 4. (Phase 6) Tạo App Database `tradingbot_app`

Phase 6 (Web Control Panel) dùng 1 DB riêng tách khỏi DB OHLCV. Sau khi Docker stack đã chạy:

```bash
# Tạo DB tradingbot_app (chỉ chạy 1 lần)
docker exec -it tradingbot_timescaledb psql -U postgres -c "CREATE DATABASE tradingbot_app;"

# Apply migrations
alembic -c alembic_app.ini upgrade head
```

> Role separation (tạo `tradingbot_app_user` với grant tách bạch) deferred sang Phase 6 slice 9 (deployment).
