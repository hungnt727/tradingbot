...
class EmaRsiReversalSignalBot:
    ...
    def __init__(
        ...
        self.strategy_1d = EmaRsiReversalStrategy(max_distance_candles=n_1d, use_ema_filter=True, min_ema_rsi=40.0)
        self.strategy_1h = EmaRsiReversalStrategy(max_distance_candles=m_1h, min_gap=3.0, min_ema_rsi=50.0)
    ...
    def _format_signal_message(self, signals: list[dict]) -> str:
        ...
        for sig in sorted_signals:
            ...
            # Thông số tối ưu từ Backtest (+710% PnL)
            tp1 = sig['price_1h'] * 0.90
            tp2 = sig['price_1h'] * 0.80
            sl = sig['price_1h'] * 1.05
            
            msg += (
                f"🔴 <b>{symbol} - SHORT</b>\n"
                f"   💰 Entry: ${price}\n"
                f"   🎯 Target 1 (10%): ${self._format_price(tp1)}\n"
                f"   🎯 Target 2 (20%): ${self._format_price(tp2)}\n"
                f"   🛡️ Stop Loss (5%): ${self._format_price(sl)}\n"
                f"   ⚠️ <i>Giữ nguyên SL khi đạt Target 1 (Gồng lãi tối đa)</i>\n"
