...
class PaperTradingEngine:
    ...
    def _process_symbol(self, symbol: str):
        ...
        # 1. Fetch latest data (enough candles for indicator warmup)
        df = self.db.query_latest_ohlcv(
        ...
        if trade.side == TradeSide.LONG:
            ...
            # Check TP
                ...
                if not trade.tp1_hit:
                    # TP1 Hit -> Move SL to entry (unless strategy says no), Update TP to TP2
                    ...
                    no_move = getattr(self.strategy, 'no_move_sl_after_tp1', False)
                    new_sl = trade.sl_price if no_move else trade.entry_price
                    self.portfolio.update_trade(trade.id, sl_price=new_sl, tp_price=tp2_price)
                else:
                    # TP2 Hit -> Close
